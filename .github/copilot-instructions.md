...
- **Tailwind CSS**: The project uses Tailwind CSS v4 via the standalone CLI (no Node.js). Theme tokens and design system colors are defined in `core/static/core/css/input.css` using `@theme`. Run `make tw-build` for a one-shot build or `make tw-watch` during development. New templates should extend `base_tw.html` and use Tailwind utility classes. See `.github/instructions/ui-ux.instructions.md` for the design system.
